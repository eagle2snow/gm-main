...
public class ModelAppInfoController extends BaseAdminController{

	private final static String path = "admin/appinfo/";
	...
	@RequiresPermissions("admin:appinfo:add")
	...
	public Map<String, Object> addAction(AppInfo model) {
		Map<String, Object> map = new HashMap<>();
		if (appInfoService.save(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@ResponseBody
	public Map<String, Object> updateAction(AppInfo model) {
		Map<String, Object> map = new HashMap<>();
		if (appInfoService.update(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@RequiresPermissions("admin:appinfo:show")
	...
	public String manager(ModelMap map, @PathVariable Integer pageIndex, @PathVariable Integer pageSize,String k) {
		DetachedCriteria dc = DetachedCriteria.forClass(AppInfo.class);
		if (!StringUtil.strNullOrEmpty(k)) {
			dc.add(Restrictions.ilike("name", k.trim(),MatchMode.ANYWHERE));
		}
		Page<AppInfo> list = appInfoService.list(dc, pageIndex, pageSize);
		map.put("page", list);
		map.put("path", path);
		map.put("key", k);
		return path + "list";
	...
	@RequiresPermissions("admin:appinfo:delete")
	@RequestMapping("deleteById/{id}.json")
	@ResponseBody
	public Map<String, Object> delete(@PathVariable Integer id) {
		Map<String, Object> map = new HashMap<>();
		if (appInfoService.deleteById(id, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public Map<String, Object> delete(@PathVariable String ids) {
		...
		if (appInfoService.deleteByIds(arrayId, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public Map<String, Object> updatePVById(@PathVariable String p, @PathVariable Integer v, @PathVariable Integer id) {
		Map<String, Object> map = new HashMap<>();
		if (appInfoService.update(p, v, id)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
