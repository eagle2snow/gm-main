...
public class AdminNewsCatController extends BaseAdminController{

	private final static String path = "admin/newscat/";
	...
	@RequiresPermissions("admin:newscat:add")
	...
	public Map<String, Object> addAction(NewsCat model) {
		Map<String, Object> map = new HashMap<>();
		if (newsCatService.save(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@ResponseBody
	public Map<String, Object> updateAction(NewsCat model) {
		Map<String, Object> map = new HashMap<>();
		if (newsCatService.update(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@RequiresPermissions("admin:newscat:show")
	...
	public String manager(ModelMap map, @PathVariable Integer pageIndex, @PathVariable Integer pageSize,String k) {
		DetachedCriteria dc = DetachedCriteria.forClass(NewsCat.class);
		if (!StringUtil.strNullOrEmpty(k)) {
			dc.add(Restrictions.ilike("name", k.trim(),MatchMode.ANYWHERE));
		}
		Page<NewsCat> list = newsCatService.list(dc, pageIndex, pageSize);
		map.put("page", list);
		map.put("path", path);
		map.put("key", k);
		return path + "list";
	...
	@RequiresPermissions("admin:newscat:delete")
	@RequestMapping("deleteById/{id}.json")
	@ResponseBody
	public Map<String, Object> delete(@PathVariable Integer id) {
		Map<String, Object> map = new HashMap<>();
		if (newsCatService.deleteById(id, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public Map<String, Object> delete(@PathVariable String ids) {
		...
		if (newsCatService.deleteByIds(arrayId, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public Map<String, Object> updatePVById(@PathVariable String p, @PathVariable Integer v, @PathVariable Integer id) {
		Map<String, Object> map = new HashMap<>();
		if (newsCatService.update(p, v, id)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
