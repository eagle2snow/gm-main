...
public class AdminCommentController extends BaseAdminController {
	...
	private final static String path = "admin/comment/";
	...
	@RequiresPermissions("admin:comment:add")
	...
	public Map<String, Object> addAction(Comment model) {
		Map<String, Object> map = new HashMap<>();
		if (commentService.save(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@ResponseBody
	public Map<String, Object> updateAction(Comment model) {
		Map<String, Object> map = new HashMap<>();
		if (commentService.update(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public String manager(ModelMap map, @PathVariable Integer pageIndex, @PathVariable Integer pageSize, String k) {
		...
		Page<Comment> list = commentService.list(dc, pageIndex, pageSize);
		map.put("page", list);
		map.put("path", path);
		map.put("key", k);
		return path + "list";
	...
	public Map<String, Object> updatePVById(@PathVariable String p, @PathVariable Integer v, @PathVariable Integer id) {
		Map<String, Object> map = new HashMap<>();
		if (commentService.update(p, v, id)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
