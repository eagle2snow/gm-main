...
public class AdminStoreController extends BaseAdminController {

	private final static String path = "admin/store/";
	...
	@RequiresPermissions("admin:store:add")
	public String addView(ModelMap map) {

		map.put("path", path);
		map.put("adminList", adminService.list());
		return path + "add";
	}
	...
	public Map<String, Object> addAction(Store model) {
		Map<String, Object> map = new HashMap<>();
		if (storeService.save(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public String updateView(@PathVariable Integer id, ModelMap map) {
		Store model = storeService.get(id);
		map.put("path", path);
		map.put("model", model);
		map.put("adminList", adminService.list());
		return path + "update";
	...
	@ResponseBody
	public Map<String, Object> updateAction(Store model) {
		Map<String, Object> map = new HashMap<>();
		if (storeService.update(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@RequiresPermissions("admin:store:show")
	...
	public String manager(ModelMap map, @PathVariable Integer pageIndex, @PathVariable Integer pageSize, String k) {
		...
		Page<Store> list = storeService.list(dc, pageIndex, pageSize);
		map.put("page", list);
		map.put("path", path);
		map.put("key", k);
		return path + "list";
	...
	public Map<String, Object> delete(@PathVariable Integer id) {
		Map<String, Object> map = new HashMap<>();
		if (storeService.deleteById(id, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public Map<String, Object> delete(@PathVariable String ids) {
		...
		if (storeService.deleteByIds(arrayId, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
