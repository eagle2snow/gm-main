...
public class AdminAdvController extends BaseAdminController {

	private final static String path = "admin/adv/";

	@Resource
	private IAdvService advService;

	@RequestMapping("add.htm")
	@RequiresPermissions("admin:adv:add")
	...
	public Map<String, Object> addAction(Adv model) {
		Map<String, Object> map = new HashMap<>();
		if (advService.save(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@ResponseBody
	public Map<String, Object> updateAction(Adv model) {
		Map<String, Object> map = new HashMap<>();
		if (advService.update(model)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	@RequiresPermissions("admin:adv:show")
	...
	public String manager(ModelMap map, @PathVariable Integer pageIndex, @PathVariable Integer pageSize, String k) {
		DetachedCriteria dc = DetachedCriteria.forClass(Adv.class);
		if (!StringUtil.strNullOrEmpty(k)) {
			dc.add(Restrictions.ilike("name", k.trim(), MatchMode.ANYWHERE));
		}
		Page<Adv> list = advService.list(dc, pageIndex, pageSize);
		map.put("page", list);
		map.put("path", path);
		map.put("key", k);
		return path + "list";
	...
	@RequiresPermissions("admin:adv:delete")
	@RequestMapping("deleteById/{id}.json")
	@ResponseBody
	public Map<String, Object> delete(@PathVariable Integer id) {
		Map<String, Object> map = new HashMap<>();
		if (advService.deleteById(id, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public Map<String, Object> delete(@PathVariable String ids) {
		...
		if (advService.deleteByIds(arrayId, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
