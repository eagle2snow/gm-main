...
<script src="/static/plugins/layerui/layui.js" charset="utf-8"></script>
			...
			.use(
					...
					function() {
						...
						layim.config({
							...
							,
							uploadImage : {
								url : '/admin/msg/chat/upload?type=img' //（返回的数据格式见下文）
								,
								type : '' //默认post
							},
							uploadFile : {
								url : '/admin/msg/chat/upload?type=files' //（返回的数据格式见下文）
								,
								type : '' //默认post
							}
						...
						layim.on('ready', function(res) {
							layim.msgbox(${noReadCount}); //模拟消息盒子有新消息，实际使用时，一般是动态获得
						});
						//监听发送消息
						layim.on('sendMessage', function(data) {
							var To = data.to;
							stompClient.send("/wsk/sendToUser", {}, JSON
									.stringify({
										'fromId' : '${curUser.id}',
										'toId' : To.id,
										'content' : data.mine.content,
										'fromName' : '${curUser.name}',
										'fromAvatar' : '${curUser.avatar}'
									}));
