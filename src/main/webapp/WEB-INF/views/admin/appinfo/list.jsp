...
<body class="hold-transition skin-blue sidebar-mini">
					...
					<div class="col-md-12">
						...
						<div class="box box-info">
							<div class="box-header with-border">
								<h3 class="box-title">app更新包列表</h3>
							</div>
							<!-- /.box-header -->
							<div class="box">
										...
										<tbody>
											...
											<c:forEach items="${page.list}" var="model">
												<tr>
													...
													<th>${model.versionIndex}</th>

													<th>${model.url}</th>

													<th><c:choose>
															<c:when test="${model.type==1}">用户版</c:when>
															<c:when test="${model.type==2}">商家版</c:when>
														</c:choose></th>

													<th><c:choose>
															<c:when test="${model.platform==1}">安卓</c:when>
															<c:when test="${model.platform==2}">苹果</c:when>
														</c:choose></th>

													<th><c:choose>
															<c:when test="${model.typeApp==1}">wgt</c:when>
															<c:when test="${model.typeApp==2}">apk</c:when>
														</c:choose></th>
	...
	<script>
		...
		function edit(id){
				...
				openPerRe("编辑app更新包", 90,90,'${adp}update/'+id+'.htm');
